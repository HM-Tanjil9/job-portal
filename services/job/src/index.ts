...
async function initDB() {
  try {
    ...
    console.log("Job service tables created successfully.");
  } catch (error) {
    console.log("Error creating job service tables", error);
    process.exit(1);
  }
}
initDB().then(() => {
  app.listen(process.env.PORT, () => {
    console.log(
      `Job service is running on port http://localhost:${process.env.PORT}`,
    );
  });
});
