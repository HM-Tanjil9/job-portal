...
const JobCard: React.FC<JobCardProps> = ({ job }) => {
  ...
  return (
    <Card className="w-full max-w-[380px] h-full flex flex-col hover:shadow-lg transition-all duration-300 border-2 hover:border-blue-500 group">
      <CardHeader className="space-y-4 pb-4 flex-shrink-0">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 text-base font-semibold">
            ...
            <span>৳ {job.salary} P.M</span>
          </div>
        </div>
      </CardHeader>
      <CardContent className="flex flex-col gap-3 pt-4 border-t mt-auto min-h-16 justify-center">
        <div className="flex w-full gap-2">
          ...
          {user && user.role === "jobseeker" && (
            <>
              {applied ? (
                <div className="flex-1 flex items-center justify-center gap-2  text-green-600 font-medium text-sm bg-green-100 dark:bg-green-800/30 rounded-md px-3 py-2">
        ...
        {job.is_active === false && (
          <div className="w-full text-center text-sm text-red-600 bg-red-100 dark:bg-red-800/30 rounded-md px-3 py-2 font-medium">
            Position closed
          </div>
        )}
      </CardContent>
